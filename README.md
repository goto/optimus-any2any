...
| | FILE__JSONPATH_SELECTOR | If set, the record will be selected based on the given JSONPath selector. Empty for ignore |
| | FILE__CSV_DELIMITER | Delimiter for CSV file format. (default: ,) |
| MC | MC__CREDENTIALS | Credentials for MaxCompute. |
| | MC__DESTINATION_TABLE_ID | Destination table ID in Maxcompute. |
...
| | S3__CSV_DELIMITER | Delimiter for CSV file format. (default: ,) |
...
| | SFTP__CSV_DELIMITER | Delimiter for CSV file format. (default: ,) |
...
| | SMTP__CSV_DELIMITER | Delimiter for CSV file format. (default: ,) |
